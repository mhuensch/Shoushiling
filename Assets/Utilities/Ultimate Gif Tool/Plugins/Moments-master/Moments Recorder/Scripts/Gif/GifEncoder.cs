...
// Tangled Reality Studios - 9/6/18 - Added option to set the transparency color
namespace Moments.Encoder
{
    public class GifEncoder
    {
        ...
        public int m_FrameDelay = 0;               // Frame delay (milliseconds)
        ...
        protected void GetImagePixels()
        {
            ...
            for (int th = m_CurrentFrame.Height - 1; th >= 0; th--)
            {
                for (int tw = 0; tw < m_CurrentFrame.Width; tw++)
                {
                    ...
                    if (transparencyColorSet && color.a == 0f)
        ...
        protected void AnalyzePixels()
        {
            ...
            NeuQuantAbstract nq;
            if (transparencyColorSet)
            {
                int[] transparencyColorArray = new int[] { transparencyColor.r, transparencyColor.g, transparencyColor.b };
                nq = new TransparentNeuQuant(m_Pixels, len, transparencyColorArray, (int)m_SampleInterval);
            }
            else
        ...
        protected void WriteGraphicCtrlExt()
        {
            ...
            m_FileStream.WriteByte(Convert.ToByte(0 |     // 1:3 reserved
                                                  (transparencyColorSet ? 8 : 0) |     // 4:6 disposal
                                                  0 |     // 7   user input - 0 = none
                                                  (transparencyColorSet ? 1 : 0)));    // 8   transparency flag
            WriteShort(m_FrameDelay); // Delay x 1/100 sec
            m_FileStream.WriteByte(Convert.ToByte(255)); // Transparent color index
