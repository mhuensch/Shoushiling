...
// Tangled Reality Studios - 10/16/18 - Modified to use TRS.CaptureTools.Extras
// Tangled Reality Studios - 02/06/20 - Modified to ignore warnings
...
public class UniGifImage : MonoBehaviour
{
    ...
    // Tangled Reality Studios - 10/16/18 - Modified to add AspectFitScript as alternative to UniGifImageAspectController
...
#pragma warning restore 0649
    ...
    private void Start()
    {
        // Tangled Reality Studios - 7/11/18 - Modified to get required UniGifImageAspectController or AspectFitScript component (and remove extra parenthesis
        if (m_rawImage == null)
            m_rawImage = GetComponent<RawImage>();
        if (aspectFitScript == null)
            aspectFitScript = GetComponent<AspectFitScript>();
        if (m_imgAspectCtrl == null)
            m_imgAspectCtrl = GetComponent<UniGifImageAspectController>();
        ...
        using (WWW www = new WWW(path))
        {
            ...
            yield return StartCoroutine(UniGif.GetTextureListCoroutine(www.bytes, (gifTexList, loopCount, width, height) =>
            {
                if (gifTexList != null)
                {
                    ...
                    // Tangled Reality Studios - 10/16/18 - Modified to add AspectFitScript as alternative to UniGifImageAspectController and check if either is null before use
                    if (aspectFitScript != null)
                        aspectFitScript.SetTexture(m_gifTextureList[0].m_texture2d, false);
                    else if (m_imgAspectCtrl != null)
